...
- `prune [--flaky] [--branches] [--all] [--stale-edges]` — `PruneCommand`: `--flaky` → `Quarantine::clear()` + save; `--branches` → `pruneMissingBranches(git branchNames ∪ default)`; `--all` → delete `graph.json`, `flaky.json`, `last-run.json`, `divergence.json`, `runs/`; `--stale-edges` → `Graph::pruneMissingDependencies()`: drops a single dependency edge whose file no longer exists on disk, for a test file that still exists (never a whole test's edge set — that is `pruneMissingTestFiles()`'s job when the test file itself is gone); no flag → `pruneMissingTestFiles()` + `pruneResultsForMissingFiles(each branch)` + `--branches`. `--stale-edges` is opt-in only, never folded into the no-flag bundle: unlike deleted test files or unreachable branches (purely historical, zero effect on cache validity), a dependency edge is exactly what a changed file is matched against to decide whether a cached result can still be trusted (`Select\Rules\PhpEdgeRule` / `Graph::testFilesDependingOn()`), so dropping one wrongly is the specific mistake `Graph::unionEdges()` was written to prevent — it stays behind an explicit flag the user opts into, same as `--flaky`. Never triggered by "not observed in the last recording pass": coverage attribution is first-loader-wins and can vary run to run even across complete, full-suite passes (`Graph::unionEdges()`'s docblock), so only a dependency's file being confirmed absent from disk (`is_file()`) counts as stale. Prints what was removed. Exit 0.
  ...
  Every local `prune` additionally collects two content-key-addressed local stores that nothing used to collect (docs/proposals/remote-layout.md §§3-6, docs/DECISIONS.md D-044). Unconditional, because none of the flags above bear on which keys this machine can address, and both sweeps print what they did. Read **after** the block above, deliberately: a branch, test file or edge this same run just pruned is already excluded from what counts as reachable, so one call cascades instead of leaving work for a second.

  - **`<stateDir>/remote/cache/objects/<k>.json`** — reachable = `Graph::addressableKeys()` (every `result['key']` under *any* branch's baseline). Unreachable entries are **truncated to zero bytes**, not unlinked: the file does two jobs on different parts of itself, `putObject()` testing `is_file()` for its "already published" marker and `object()` decoding the contents, so a zero-byte file keeps the marker true, fails the decode, frees the blocks, re-publishes nothing, and refills on the next lookup. `--forget-published` unlinks instead, reclaiming the inode for a redundant `put` (idempotent — objects are immutable), and stays behind a flag because on `remote_push: objects` that re-publication is noise on a shared remote.
  - **`<stateDir>/coverage/<k>.cov`** — a *different* key space: `CoverageSnapshots` keys by `ContentHash::of()` of the test file, never by a content key, because a content key needs a `Graph` that does not exist inside the PHPUnit child process in filtered mode. Reachable = `ContentHash::of()` of each `Graph::allTestFiles()` entry at its **current** on-disk content; anything else is permanently stale (the file changed, so its key can never be recomputed, or it is gone). No marker role, so these are unlinked. Safe against `allTestFiles()` because the only thing that ever removes an `edges` key is `pruneMissingTestFiles()`, gated on `!is_file()`, and `markKnownTestFiles()` seeds an empty entry for a test file with zero edges.

  `status` reports the mirror as `mirror:    1452 objects · 726 reachable · 2.4 MB reclaimable`, computed regardless of whether a baseline exists. **Its figure and `prune`'s can legitimately disagree**: `status` reports reachability as it stands, while `prune` prunes first and collects after, so a run that drops the last baseline referencing a branch reports `kept 0` against a `status` that had said otherwise moments earlier. That is the cascade working, and it costs a refetch, not data.
...
- `remote:init [--name=] [--owner=] [--branch=] [--same-repo] [--no-create] [--dry-run]` — `RemoteInitCommand`: one-shot setup of the shared remote cache (docs/sharing-the-cache.md "Setup: dedicated git repository", D-038/D-045). Origin: `Change\Git::originUrl()` = `config --get remote.origin.url`, deliberately not `remote get-url`, whose `insteadOf` expansion would not be the string `ProjectKey::originIdentity()` hashes into the shared project key → `Cache\Remote\OriginUrl::parse()` → `{url, host, owner, name}` for the scp-like, `https`, `ssh`, `git+*` and local `file://`/absolute-path forms `GitRemoteCache` documents (`owner` keeps every path segment but the last, so a GitLab subgroup survives instead of being flattened into the name). No `origin`, or one it cannot split → message explaining that the cache keys every project by origin identity, exit 1. Proposal: `origin->sibling($owner, $name . '-replay-cache')`, from origin's repository name and never `basename($root)`; always `.git`-suffixed, because `RemoteCacheFactory::looksLikeGit()` routes a suffix-less `https://…/owner/name` to `HttpRemoteCache`. `--public` does not exist: refused with one line, exit 2 (`Command::INVALID`) — the cache holds test names, file paths and failure messages. Existence before creation, so the idempotent case costs nothing and calls nothing: `git ls-remote` (exit 0, empty output included) → "already there" and `gh` is never invoked; otherwise `gh repo create <owner>/<name> --private` behind `ExecutableFinder` + `gh auth status`, and a missing/unauthenticated/refused `gh` prints the manual steps and continues rather than aborting. No token is ever asked for, read, accepted or stored (`gh` owns one credential, git the other). Probe (the point of the command — "configured" and "works" are different claims): a `{probe, at}` body at `objects/<current shard>/replay-probe-<16 hex>.json`, `put()` + `end()` through a `RemoteCacheFactory`-built backend rooted at `<stateDir>/remote-init/write`, read back through a **second** instance rooted at `<stateDir>/remote-init/verify` (a mirror is bound to the URL it cloned, so only a fresh one proves the remote was reached), compared byte for byte, then deleted **through that second instance** — a delete on the writer is silently undone by `GitRemoteCache::adoptFetchedHistory()`'s `reset --hard` plus its `put()` buffer replay, which the writer of a branch that did not exist upstream always reaches in `end()`. Both scratch mirrors are removed afterwards; the project's real mirror at `<stateDir>/remote/git` is never touched (it may still be a mirror of a previously configured remote). Config: writes `remote` / `remote_branch` / `remote_push => 'off'` with a per-key *why* comment when `phpunit-replay.php` is absent (`AtomicFile::write`); when it exists, prints those lines and never edits the file, which may already carry `default_branch`/`baseline_branches`/`watch`. `--same-repo` (D-045): skips creation entirely, configures `origin->canonical()` with `remote_branch` defaulting to `phpunit-replay-cache`, and prints the clone-size cost (every plain `git clone` of the project fetches the cache branch) in the output rather than leaving it in a document; a `--branch` equal to `currentBranch()` or to the resolved default branch is refused with exit 2 before anything runs, because the probe would otherwise commit onto a code branch (the two branches this can be certain about are the only ones refused — an explicit choice beyond them gets a printed warning instead). CI: the deploy key with write access (a GitHub deploy key is SSH-only), the matching secret name and a workflow snippet are **printed and never performed** — no keys generated, nothing uploaded, no secret set. `--dry-run` contacts nothing and writes nothing, which is also what makes every test of this command network-free. Exit 0 only when the probe made the round trip, 1 otherwise (the backend's `lastError()` is printed verbatim).
