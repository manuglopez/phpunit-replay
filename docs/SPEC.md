...
Partial PHPUnit options (`--filter`, `--exclude-filter`, `--group`, `--exclude-group`, `--testsuite`, `--exclude-testsuite`, an explicit path, `--covers`, `--uses`) **disable selection**: whatever the user asked for runs, with the extension in `results-only` mode. `results-only` persists **nothing at all** — no edges, no results, no sha, no pruning, no quarantine write, no remote publish (`Cache\GraphUpdater::apply()` is never even called for such a pass, in `run` or in `verify`): a CLI selection means "run exactly this," and nothing observed under it may enter the cache. XML-declared group/testsuite configuration (a project's own `<groups><exclude>` or `defaultTestSuite`) is never a selection under this paragraph — it is the project's own definition of what "the whole suite" means, so a `phpunit.xml` that excludes a group does not, by itself, disable selection (`PHPUnit\ConfigurationReader::hasPartialSelection()` answers from the CLI half of the configuration alone). `--random-order` with a different seed changes which test is the first in its process to load a given class, enum or const file, and therefore which test the file's load-time execution is attributed to — the top level of a file runs once per process, so only that first test sees it. Edges being file-level does not make this harmless on its own; what does is that a re-record **unions** edges instead of replacing them (SPEC §7.2), so a different order can only add an attribution, never take one away. `--order-by=random` with no seed is accepted as-is. This whole paragraph is specific to `run`; `record` (§3.3) refuses a partial selection instead of degrading to `results-only`, since a partial run cannot produce record's one product, a complete baseline; `verify` (§12.2) takes the same selection and still reports what it can (PHPUnit's own exit code, and a `would replay` figure read off the baseline as it already was) without persisting anything either, for the same reason.
...
- If `PHPUnit\ConfigurationReader::hasPartialSelection()` is true — the CLI half of the configuration alone (`hasFilter()`, `hasExcludeFilter()`, `hasGroups()`, `hasExcludeGroups()`, `hasTestSuite()`, `hasExcludedTestSuite()`, `arguments()` with a path), reconstructed here from `$_SERVER['argv']` since this caller has no wrapper-known argument list and PHPUnit's own `$configuration` is already merged (XML plus CLI, provenance gone) — → `results-only`, which persists nothing at all (§3.1). When `mode=record` specifically (a standing "always record" configuration, unlike `auto` landing on `record` only for lack of a baseline yet) is what the selection defeated, the summary line notes it (`record mode: baseline NOT refreshed (partial selection)`) instead of staying silent — the same guarantee record's own CLI command (§3.3) enforces by refusing outright, applied here as visibility instead, since a standing config is not a one-shot command and downgrading to `results-only` is still the correct behaviour for an ordinary filtered run.
...
- Truncated run (recording or replaying, but the process was cut short — a crash or a signal, not a CLI selection): merges results for already-known test files; no sha, no pruning, no edges.
- Partial CLI selection / results-only run (§3.1): merges **nothing** — `GraphUpdater::apply()` is never called at all for such a pass, so no sha, no pruning, no edges, and no results either, regardless of whether the test file was already known to the graph.
...
A `verify` carrying a partial CLI selection persists **nothing at all**, for the same reason §3.1 gives for `run`: `GraphUpdater::apply()` is never called for such a pass, so the graph, the quarantine table, the baseline sha and `divergence.json`'s lifetime counters are all left completely untouched. `would replay` is still reported (the pure, read-only figure described above, unaffected by whether this pass persists anything), `divergences` is always `0` and `unverified` always equals `would replay` — this pass deliberately did not check any of them, rather than silently implying a clean full-suite verification it never performed. `tests`/the exit code still reflect what PHPUnit actually ran.
